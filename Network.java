...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public boolean addFollowee(String name1, String name2) {
        if(this.getUser(name1)==null) return false;
        if(this.getUser(name2)==null) return false;
    ...
    public String mostPopularUser() {
        ...
        int maxFollowers = 0;
        for(int i=0;i<userCount;i++){
            if(this.followeeCount(users[i].getName())>maxFollowers){
                popularUser = users[i];
                maxFollowers = this.followeeCount(users[i].getName());
            }
        }
    ...
    public String toString() {
        String ans = "Network:" + "\n";
        for (int i=0;i<userCount;i++) {
            if(i==userCount-1) ans = ans +users[i];
            else ans = ans + (users[i]) + "\n";
        }
        return ans;
    }
}
